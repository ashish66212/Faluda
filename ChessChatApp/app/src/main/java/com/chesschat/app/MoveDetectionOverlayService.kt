...
class MoveDetectionOverlayService : Service() {
    ...
    private fun loadSettings() {
        ...
        addLog("loadSettings", "Board configured: $boardAutoDetected")
    }
    ...
    private fun saveSettings() {
        ...
        prefs.edit()
            ...
            .apply()
        addLog("saveSettings", "Settings saved (API: $ngrokUrl, Configured: $boardAutoDetected)")
    }
     ...
     * 1. Detects board position and orientation (or uses manual setup if already configured)
    ...
    private fun startAutomaticWorkflow() {
        ...
        if (boardAutoDetected && boardX > 0 && boardY > 0 && boardSize > 0) {
            addLog("startAutomaticWorkflow", "✓ Using MANUAL board position (skipping auto-detection)")
            addLog("startAutomaticWorkflow", "  Position: X=$boardX, Y=$boardY, Size=$boardSize")
            addLog("startAutomaticWorkflow", "  Flipped: $isFlipped")
            
            val manualColor = if (!isFlipped) "white" else "black"
            ...
            handler.post {
                ...
                handler.postDelayed({
                    ...
                    sendStartCommandAutomatic { success ->
                        if (success) {
                            // Send manual color to API
                            handler.postDelayed({
                                addLog("startAutomaticWorkflow", "Step 2: Sending color '$manualColor' to API...")
                                sendColorCommandAutomatic(manualColor) { colorSuccess ->
                                    if (colorSuccess) {
                                        ...
                                        handler.postDelayed({
                                            ...
                                            startDetection()
                                            addLog("startAutomaticWorkflow", "=== AUTOMATIC WORKFLOW COMPLETE (MANUAL BOARD) ===")
                                        }, 500)
                                    } else {
        ...
        // Board not manually configured - run automatic detection
    ...
    private fun createCompactOverlay() {
        ...
        // Copy logs button
        val copyLogsButton = overlayView.findViewById<TextView>(R.id.copyLogsButton)
...
private fun cancelManualSetup() {
    ...
    private fun copyLogsToClipboard() {
        try {
            ...
            val clip = android.content.ClipData.newPlainText("Chess App Logs", logBuffer.toString())
    ...
    override fun onDestroy() {
        ...
        compactOverlay?.let { 
            try {
                ...
                Log.e(TAG, "Error removing compact overlay: ${e.message}")
            }
        }
        expandedOverlay?.let { 
            try {
                ...
                Log.e(TAG, "Error removing expanded overlay: ${e.message}")
            }
        }
        manualSetupOverlay?.let { 
            try {
        ...
        try {
            ...
            Log.e(TAG, "Error shutting down client: ${e.message}")
        }
